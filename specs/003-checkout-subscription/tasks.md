...
- [ ] T006 [US1] `src/app/(customer)/checkout/page.tsx` + `checkout-form.tsx`: summary from the basket, site confirmation, installation-day picker from the mirror, billing email prefilled, place order via `create_subscription` passing the displayed total; on `price changed` show the new total and a confirm step; on success clear the basket and go to the subscription
- [ ] T007 [US1] Basket "Continue" navigates to `/checkout`; disabled without a site

## Phase 3: US2 — Invoice payment

- [ ] T008 [US2] `src/app/(customer)/subscriptions/[id]/page.tsx`: lines, totals, status, installation and end dates, and for pending: reference, amount due, bank details
- [ ] T009 [US2] `src/app/(ops)/ops/subscriptions/page.tsx` + `manager.tsx`: list all, mark paid, cancel or release with a note, edit bank details

## Phase 4: US3 — See and cancel

- [ ] T010 [US3] `src/app/(customer)/subscriptions/page.tsx` list; header link
- [ ] T011 [US3] Cancel action on the detail page for pending subscriptions; active ones explain the operator path

## Phase 5: Polish

- [ ] T012 Full gate and the seven quickstart checks; README status
